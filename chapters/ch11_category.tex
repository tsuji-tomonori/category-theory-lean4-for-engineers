...
図\ref{fig:ch11-category-overview}では、型を点、関数を矢印として置き、恒等関数と型が接続する関数合成を同じネットワーク上に示す。
...
プログラム上では、\texttt{id} や \texttt{fun x => x} に相当する。入力をそのまま返す middleware や無効化された adapter も、このモデルでは恒等射として読める。
...
図\ref{fig:ch11-composition-order}では、入力がまず (f) を通り、その出力が (g) の入力になることを矢印の向きで確認できる。
...
図\ref{fig:ch11-associativity}では、三つの射の順序を (f,g,h) のまま保ち、前半と後半のどちらを先に一つの射としてまとめるかだけを変える。
...
これを厳密に一つの圏として定式化するには、対象にする型の universe level を管理する必要があるが、本章のコードはその構造を包装せず、関数合成の法則だけを点ごとに検査する。
...
図\ref{fig:ch11-noop-removal}では、\texttt{pipelineLeft} の後ろに恒等処理を合成した経路と、合成しない経路が同じ出力へ至る。

\FigurePlaceholder{fig-ch11-noop-removal.png}{0.88}{恒等処理の除去}{fig:ch11-noop-removal}

\section{圏を定義として見る}

圏、恒等射、合成、結合律の標準的な定義と記法については、Leinster~\cite{leinster-basic-category-theory}およびRiehl~\cite{riehl-category-theory-in-context}を基準とする。本章の型と関数の例はその特殊例であり、一般の圏の射が関数であるとは限らない。
...
対象、射、恒等射、合成と二つの法則が圏の最小語彙である。\(g \circ f\) は先に\(f\)、次に\(g\)と読む。型と純粋関数は重要な例だが、一般の圏の対象を型、射を関数と無条件に同一視しない。
