...
\FigurePlaceholder{fig-ch26-price-pipeline.png}{0.90}{料金計算を、割引・手数料・税・丸めの処理パイプラインとして見る}{fig:ch26-price-pipeline}
...
\begin{syntaxbox}
\texttt{import} は、別ファイルやライブラリで定義された名前を使うための構文である。
\begin{itemize}
\item \texttt{import A.B.C} と書くと、\texttt{A.B.C} が提供する定義や定理をこのファイルで参照できる。
\item Mathlib を使う章では、必要なライブラリを \texttt{import} してから定理を書く。
\end{itemize}
\end{syntaxbox}
...
\FigurePlaceholder{fig-ch26-commuting-operations.png}{0.90}{固定手数料と固定税は可換だが、割引と手数料は一般には可換でない}{fig:ch26-commuting-operations}
...
\FigurePlaceholder{fig-ch26-rounding-boundary.png}{0.90}{明細ごとの丸めと合計後の丸めでは結果が変わりうる}{fig:ch26-rounding-boundary}
