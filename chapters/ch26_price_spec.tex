...
図\ref{fig:ch26-price-pipeline}では、割引、手数料、税、丸めを別々の処理として並べ、順序と丸め位置を仕様の一部として表す。
...
また、型は \texttt{rateDen = 0} を排除していない。
本章の反例は分母 \texttt{10} だけを使うが、一般の税率 API にするなら分母が正である証拠を引数または型に含める必要がある。
...
図\ref{fig:ch26-commuting-operations}の左側は加算どうしなので可換である。
右側は \texttt{Nat} の切り捨て減算が途中で \texttt{0} に達するため、二経路が一致しない場合を表す。
...
次に、本章の \texttt{Nat} モデルで順序交換が常には成り立たない例を見る。
...
この不一致の直接の原因は、\texttt{Nat} の減算が \texttt{0} で止まることである。
\texttt{discount <= amount} という条件のもとでは、この固定額割引と固定手数料は順序交換できる。
符号付き整数上の固定額減算でも同じ反例は生じない。
手数料を割引対象に含める業務差を表したい場合は、率や上限を持つ別の割引関数をモデル化する必要がある。
...
このコードでは、明細ごとに税を計算する\texttt{taxPerLine2}と、合計してから税を計算する\texttt{taxOnSum2}を分けて定義している。
\texttt{\#eval}の二行は具体例の結果を観察するためのもので、\texttt{rounding\_position\_counterexample}はその二つが等しくないことをLeanに確認させている。

図\ref{fig:ch26-rounding-boundary}は、各明細で整数除算してから足す経路と、先に合計してから一度だけ整数除算する経路を分けている。
反例は、この二経路が一般には可換でないことを示す。
...
料金計算パイプラインは \texttt{Money -> Money} の処理の合成である。
順序交換の安全性は、対応する二つの合成が同じ関数になるか、つまり図式が可換になるかで判定できる。
...
料金計算は、税、割引、手数料、丸めの順序によって結果が変わる。

固定税と固定手数料のように、足し算だけで表せる処理は順序交換を等式で証明できる。

割引と手数料、丸めと合算は、一般には順序を入れ替えられない。反例を Lean で示せる。

リファクタリング安全性は、変更前後の関数が同じ結果を返すという等式として書ける。

割引が金額を増やさない、手数料が金額を減らさない、といった性質は不等式で書く。

Lean の証明は、仕様の核を検査する道具であり、税法や外部システムを含む本番全体の保証ではない。
