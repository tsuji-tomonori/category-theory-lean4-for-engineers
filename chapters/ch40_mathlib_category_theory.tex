...
\FigurePlaceholder{fig-ch40-overview.png}{0.90}{自作ミニ定義から Mathlib CategoryTheory へ進む地図}{fig:ch40-overview}
...
\begin{syntaxbox}
Mathlib の例では、ライブラリを読むための補助構文も出てくる。
\begin{itemize}
\item \texttt{open CategoryTheory} は、\texttt{CategoryTheory} 名前空間の中にある名前を短く参照できるようにする構文である。
\item \texttt{open scoped CategoryTheory} は、そのスコープで用意された記法を有効にする。ここでは圏論用の射や合成の記法を読みやすくするために使う。
\item \texttt{section CategoryFields} は、変数宣言や補助定理を一つの範囲にまとめる囲いである。\texttt{end CategoryFields} までがその範囲になる。
\item \texttt{[Category.\{v\} C]} の角括弧は、圏構造を型クラス推論で自動的に探して使う、という指定である。
\end{itemize}
\end{syntaxbox}
...
\begin{definitionbox}
Mathlib の \texttt{Category C} は、対象型 \texttt{C} に対して、射 \(X \longrightarrow Y\)、恒等射 \(\mathbb{1}_X\)、合成 \texttt{f ≫ g}、および恒等律・結合律を与える型クラスである。本書の第11章で作った「対象・射・合成・恒等射・法則を持つもの」は、Mathlib ではこの \texttt{Category C} として表される。
\end{definitionbox}
...
この式は、第15章と第31章で見た可換図式そのものである。左辺は「先に \texttt{F} 側で処理してから adapter を通す」経路であり、右辺は「先に adapter を通してから \texttt{G} 側で処理する」経路である。両者が等しいとき、adapter は業務ロジックと干渉しない。
...
\FigurePlaceholder{fig-ch40-mini-to-mathlib.png}{0.92}{自作ミニ定義を Mathlib の語彙へ移す対応関係}{fig:ch40-mini-to-mathlib}
