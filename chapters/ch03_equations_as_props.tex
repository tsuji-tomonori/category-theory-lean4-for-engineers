...
本章では、この主張を「命題」と呼ぶことを先に体験し、\texttt{rfl} などの証明道具は次章で扱います。
\end{chapterabstract}
...
主張はまず正しく読める必要があり、実際に成り立つかどうかは、次章で \texttt{rfl} を使って確かめます。
...
本章では、等式を主張として読みました。
次章では、最初の証明道具 \texttt{rfl} を使い、\texttt{hasDiscount true = true} が閉じ、\texttt{hasDiscount false = true} が閉じない過程を比較します。
