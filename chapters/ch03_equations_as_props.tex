...
本章では、この主張を「命題」と呼び、成り立つ主張と成り立たない主張を、定義の展開によって区別します。
\end{chapterabstract}
...
したがって、\texttt{hasDiscount true} は \texttt{true} に計算され、\texttt{hasDiscount false} は \texttt{false} に計算されます。
ここでの \texttt{member} は、呼び出し時に \texttt{true} または \texttt{false} を受け取る引数です。

\texttt{\#eval hasDiscount true} は、関数を実際に\emph{計算}し、その結果の値を表示します。
これは、テストで特定の入力に対する結果を確かめる感覚に近い操作です。
続いて、この計算結果を「主張」として書き直します。
...
\texttt{\#check} で確認すると、Lean がこの等式を値ではなく、成り立つかどうかを問える\emph{命題}として扱っていることが分かります。

この一行は、表\ref{tab:ch03-reading}のように段階を分けて読めます。
段階を追うと、この主張が成り立つ理由を確認できます。
...
主張は、まず正しく読める必要があります。
定義を展開して左右を比較すると、主張が成り立つかどうかを判断できます。
