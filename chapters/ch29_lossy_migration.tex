...
移行結果が等しい旧データどうしを同値とみなす関係は作れますが、本章では商対象の普遍性までは証明していません。
...
本章では、\texttt{fullName} から \texttt{firstName} / \texttt{lastName} への移行を題材に、lossy migration と仕様弱化を扱いました。
実務の文字列処理そのものは、さらに多くの例外を持ちます。
姓名順、敬称、法人名、ニックネーム、複数空白、Unicode 正規化なども考える必要があります。
本章の Lean モデルは、それらを直接検証しているわけではありません。

それでも、この小さなモデルには価値があります。
どの仕様が強すぎるのか、どの条件なら復元できるのか、どの観測なら保存できるのかを分けて書けるためです。

\begin{warningbox}
本章の証明は、実装の文字列パーサが正しいことを保証しません。
保証しているのは、モデル化した範囲での情報損失、条件付き復元、表示互換、正規化安定性です。
実システムに適用する場合は、パーサ、Unicode 正規化、入力データ品質、運用上の対象範囲を別途確認する必要があります。
\end{warningbox}
