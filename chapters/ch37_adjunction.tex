...
図\ref{fig:ch37-free-forgetful-overview}の一方向は材料から構造を生成し、反対方向は構造を持つ対象から土台の型だけを取り出す。随伴が主張するのは、後で述べる構造保存写像の対応であり、この二操作が互いに逆関数だということではない。
...
\begin{intuitionbox}
\texttt{List A} は、\texttt{A} の値を材料として、「空」と「連結」を持つモノイドを自由に生成する。ここで「自由」とは、モノイド則から必要になる等式以外に、要素同士の同一視、順序交換、簡約を追加しないという意味である。構造の個数や包含関係について「最小」という意味ではない。
\end{intuitionbox}
...
コード \ref{lst:ch37_tiny_monoid} では、\texttt{TinyMonoid} に単位元、二項演算、三つの法則を持たせ、\texttt{listMonoid} で空リストと連結がその法則を満たすことも示している。\texttt{foldMap} は、空リストと \texttt{x :: xs} に分けてリストを \texttt{M} の値へ畳み込む。\texttt{foldMap\_singleton} は、1要素リストに対しては元の関数 \texttt{f} と同じ働きをすることを示している。
...
\texttt{foldMap\_append} は、\texttt{foldMap} がリスト連結を保存することを表している。\texttt{foldMapHom\_unique} は、同じ要素写像を持つ任意の準同型 \texttt{h} が、すべてのリストで \texttt{foldMap} と同じ値を返すことを示す。後者の結論は点ごとの関数等式であり、\texttt{MonoidHom} 構造体同士の等式を直接述べてはいないが、ここで必要な一意性を与えている。証明にはリストの帰納法を使い、空リストでは単位元保存、\texttt{x :: xs} では演算保存と帰納法の仮定を使う。
...
コード\ref{lst:ch37_foldmap_hom}の\texttt{foldMapHom\_unique}がこの一意性を示す。リストに対する帰納法を使い、空リストでは単位元保存、\texttt{x :: xs} では \texttt{[x] ++ xs} と見て、準同型が結合を保存することを使う。

\section{随伴の定義を少しだけ見る}

一般の随伴はHom集合の全単射だけでなく、両変数について自然な同型として定義される~\cite{leinster-basic-category-theory,riehl-category-theory-in-context}。
...
リストの例では、材料 \texttt{A} から \texttt{List A} を作る。DSL では、トークンや命令の集合から構文木を作る。一般の構文木は自由モノイドではなく、演算記号とその引数の個数を定めたシグネチャに対する自由代数として扱う。共通するのは、「材料から、指定した演算以外の等式を勝手に加えず構造を作り、後から意味を与える」という設計である。
...
第22章では、Galois 接続を抽象化と具体化の対応として見た。随伴を学ぶと、Galois 接続は特別な随伴として理解できる。
...
図\ref{fig:ch37-galois-adjunction}では、左辺の比較と右辺の比較が同値になることを、前順序を圏と見たときのHomの対応として表している。
...
\end{minted}
\caption{\texttt{PreorderMini}・\texttt{GaloisConnection}}
\label{lst:ch37_galois}
\end{listing}

この定義では、第22章と同様に反射律と推移律を持つ \texttt{PreorderMini} を明示的に渡す。表示した同値条件から \texttt{lower} と \texttt{upper} の単調性も導けるので、フィールドとして重ねて要求していない。
...
\section{発展読書への橋渡し}
\label{sec:ch37-further-reading}
