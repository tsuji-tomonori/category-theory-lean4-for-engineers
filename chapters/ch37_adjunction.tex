...
この章では、圏論の発展的な概念である随伴を、ソフトウェアエンジニアが利用できる直感に絞って導入します。
...
本章では、この二つが一対の操作として自然に対応する理由を、free monoid と list を通して説明します。
\end{chapterabstract}
...
\item Mathlib の本格的な随伴へ進む前に、本章のミニモデルで扱った範囲を説明できるようになります。
\end{learninggoals}
...
この二つが一対一に対応することが、free monoid の普遍性です。
\end{definitionbox}
...
第22章では、Galois 接続を抽象化と具体化の対応として捉えました。
...
この定義では、第22章と同様に、反射律と推移律を持つ \texttt{PreorderMini} を明示的に渡します。
...
随伴は非常に広い概念です。
本章では、初学者が次章以降へ進むための入口だけを扱いました。

\begin{warningbox}
本章では、Mathlib の \texttt{CategoryTheory.Adjunction}、単位と余単位の三角恒等式、随伴からモナドが生じる一般論、Kan 拡張との関係は扱いません。
これらは重要ですが、ここで一度に扱うと、自由構成と忘却という実務上の直感が見えにくくなります。
\end{warningbox}

本章で扱った内容は、次の範囲です。

\begin{itemize}
\item \texttt{List A} を free monoid として捉えます。
\item \texttt{foldMap} を、要素への関数からリスト全体の準同型を作る操作として捉えます。
\item その準同型がリスト連結を保存することを Lean で確認します。
\item Galois 接続を、前順序上の随伴として捉えます。
\end{itemize}

この範囲だけでも、随伴が「抽象数学だけの概念」ではなく、設計上の自然な対応を表す言葉であることが分かります。

\section{発展読書への橋渡し}
\label{sec:ch37-further-reading}

圏論として随伴を本格的に学ぶ場合、次に読むべき内容は目的によって変わります。

\begin{itemize}
\item 基礎から丁寧に進む場合は、随伴を早い段階で扱う入門書を読むとよいでしょう。
特に、自由構成、普遍性、極限との関係を順に確認します。
\item 現代的な数学として学ぶ場合は、随伴、モナド、Kan 拡張へ進む流れを追うとよいでしょう。
\item 応用圏論として学ぶ場合は、前順序、Galois 接続、モノイダル構造、データベースや設計問題との接続を確認するとよいでしょう。
\item Lean で本格的に扱う場合は、まず本章の自作定義を理解し、その後で Mathlib の \texttt{CategoryTheory} 名前空間に進むとよいでしょう。
\end{itemize}

本書の文脈では、次章の米田の補題へ進む前に、「対象そのものではなく、写し方の対応を見る」という感覚を持つことが重要です。
随伴も米田も、内部表現を直接扱うのではなく、外部との関係によって対象を理解するための言葉だからです。
...
本章では Mathlib の本格的な随伴 API までは扱わず、自作のミニ定義を使って直感を固めました。
