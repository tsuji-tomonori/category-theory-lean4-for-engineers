...
\FigurePlaceholder{fig-ch37-free-forgetful-overview.png}{0.90}{自由構成と忘却の概念図}{fig:ch37-free-forgetful-overview}
...
\FigurePlaceholder{fig-ch37-free-monoid-list.png}{0.90}{要素の写像からリスト全体の準同型が一意に決まる}{fig:ch37-free-monoid-list}
...
\begin{syntaxbox}
リストを再帰的に読むときは、空リストと「先頭 + 残り」に分ける。
\begin{itemize}
\item \texttt{| [] => ...} は、リストが空の場合の枝である。
\item \texttt{| x :: xs => ...} は、先頭要素 \texttt{x} と残りのリスト \texttt{xs} に分解する枝である。
\item \texttt{::} は cons と呼ばれ、値をリストの先頭に付ける構成子である。
\item この形の定義では、各枝の右側にその場合の結果を書く。
\end{itemize}
\end{syntaxbox}

コード \ref{lst:ch37_tiny_monoid} では、\texttt{TinyMonoid} に単位元、二項演算、三つの法則を持たせている。\texttt{foldMap} は、リストを \texttt{M} の値へ畳み込む。\texttt{foldMap\_singleton} は、1要素リストに対しては元の関数 \texttt{f} と同じ働きをすることを示している。
...
\end{minted}
\caption{foldMap はリスト連結を保存し、その条件を満たす拡張は一意である}
...
\FigurePlaceholder{fig-ch37-galois-adjunction.png}{0.90}{前順序における随伴としての Galois 接続}{fig:ch37-galois-adjunction}
...
\begin{syntaxbox}
\texttt{<->} は、二つの命題が同値であることを表す構文である。
\begin{itemize}
\item \texttt{P <-> Q} は、「\texttt{P} ならば \texttt{Q}」と「\texttt{Q} ならば \texttt{P}」の両方を持つ命題である。
\item 数学記号では \(\leftrightarrow\) と書かれ、「if and only if」や「同値」と読む。
\item Galois 接続では、片方の順序関係で比較できることと、もう片方の順序関係で比較できることが同値になる。
\end{itemize}
\end{syntaxbox}
