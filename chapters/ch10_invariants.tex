...
この二つを組み合わせることで、\texttt{depositValid} は「有効な口座を受け取り、有効な口座を返す」関数になります。
...
これは、先ほど定義した \texttt{WithdrawPost} で表せます。
...
本章では、場合分けの証明には立ち入りません。
場合分けは、第19章で扱います。
ここでは、成功条件があるときに \texttt{some} が返ることと、成功条件がないときに \texttt{none} が返ることを、仕様として読みます。
...
初めて Lean で仕様を書くときは、その中から一つだけ選びます。
...
これにより、仕様は人間がレビューできるだけでなく、機械でも検査できる形になります。
