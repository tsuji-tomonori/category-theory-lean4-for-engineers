...
図\ref{fig:ch10-invariant-flow}では、\texttt{structure} でデータの形を定め、述語で有効性を表し、操作がその有効性を保つことを定理で確認する。
...
本書では、不変条件を「データが有効な状態であるために満たすべき命題」と呼ぶ。Lean では、多くの場合、\texttt{AccountValid : Account -> Prop} のように値を受け取って命題を返す述語として書く。
...
\texttt{\{ a with balance := ... \}} は \texttt{balance} だけを更新した新しい値を作る。\texttt{simpa [deposit, AccountValid] using h} は二つの定義を展開し、ゴールを既存の証明 \texttt{h} と同じ形にして閉じる。
...
図\ref{fig:ch10-pre-post-invariant}では、事前条件を満たす状態に操作を適用し、事後条件を得る間も不変条件を保つ、という三者の位置関係を示す。
...
\texttt{CanWithdraw a amount} は、\texttt{amount} が \texttt{a.balance} 以下であるという命題を表す。Lean では ASCII の \texttt{<=} と数学記号の \texttt{≤} のどちらでも、この順序関係を \texttt{Prop} として書ける。
...
関数名だけではこの区別は表せないため、Lean の述語は、成功条件、失敗条件、状態更新、保持する情報を明示的に分ける役割を持つ。
...
図\ref{fig:ch10-subtype-gate}では、通常の \texttt{Account} を検査し、\texttt{AccountValid} の証明が得られた値だけを \texttt{ValidAccount} として後段へ渡す。
...
この証明は二つの部分からなる。
\texttt{constructor} で \texttt{And} のゴールを二つに分け、行頭の \texttt{·} で各部分の証明を区切っている。
...
失敗理由まで区別する必要があるなら、\texttt{Option} ではなくエラー値を持つ \texttt{Except} を選ぶ。この選択は戻り値の型に現れる。
...
要点と記法
\begin{itemize}
\item \texttt{structure 名前 where ...} \quad レコード型（ドメインモデル）の定義。
\item \texttt{A -> Prop} \quad 値が条件を満たすかを表す述語。
\item \texttt{Subtype} / \texttt{\{x // P x\}} \quad 値と「条件を満たす証明」を一緒に持つ型。
\item 事前条件は操作前の要求、事後条件は操作後の保証、不変条件は有効な状態で守る性質。
\item 失敗しうる処理は \texttt{Option} や \texttt{Except} で失敗を戻り値の型に表す。
\end{itemize}
\end{summarybox}
