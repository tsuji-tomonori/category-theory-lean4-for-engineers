...
ここまでは、\texttt{true} や \texttt{1000} のような具体的な値についての主張を扱ってきました。
次章では、「任意の価格について」「すべてのユーザーについて」といった、値を一つに固定しない主張を扱います。
そのために、引数を取る定理、\texttt{forall}、および証明を始める \texttt{intro} を導入します。
