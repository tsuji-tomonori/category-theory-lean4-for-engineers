...
図\ref{fig:ch17-overview}は、普通の値を文脈へ入れる \texttt{pure} と、文脈を保ったまま後続処理へつなぐ \texttt{bind} を中心に、失敗・エラー・状態の例を並べている。
...
圏論的には、モナドは自己関手と単位・乗法という二つの自然変換を持ち、対応する単位律と結合律を満たす構造として定義される。
本章では、それと対応する \texttt{pure} と \texttt{bind} の表示を使い、操作がすべての中身の型に一様に与えられることと三法則に焦点を絞る。
...
\end{itemize}
これらをすべての型 \texttt{A}、\texttt{B} に一様に与え、後で述べる三つのモナド則を要求する。
\end{definitionbox}
...
ここでは Lean 標準の \texttt{Option.bind} を直接使わず、動作を \texttt{match} で追える \texttt{optPure} と \texttt{optBind} を自作する。
...
図\ref{fig:ch17-bind-flow}の成功経路では \texttt{some a} の \texttt{a} が後続関数へ渡り、失敗経路では \texttt{none} がそのまま返る。
...
モナド則は実行順序の交換を許す法則ではない。
図\ref{fig:ch17-monad-laws}が示すように、同じ順序の処理列から不要な \texttt{pure} を除いたり、\texttt{bind} の括弧を付け替えたりしても意味が変わらないという法則である。
...
左単位律は定義を展開すれば \texttt{rfl} で閉じる。
右単位律と結合律は \texttt{x} を \texttt{none} と \texttt{some} に分け、\texttt{<;>} で両方のゴールへ \texttt{rfl} を適用する。
...
これは、旧データの欠損によって新スキーマへ移行できない状況の最小モデルである。
失敗可能性を返り値の \texttt{Option UserV2} に明示するため、後続処理も成功と失敗の両方を扱う必要がある。
...
図\ref{fig:ch17-migration-pipeline}では、\texttt{requireEmail} が成功したときだけ \texttt{requireAge} へ進み、どちらかが失敗すれば全体が \texttt{none} になる。
...
\texttt{let withEmail ← requireEmail u} は、成功値を \texttt{withEmail} として後続へ渡す。
\texttt{Option} の \texttt{do} ブロックは途中で \texttt{none} になれば停止するため、この記述は上の \texttt{optBind} と同じ制御フローになる。
...
このコードでは、\texttt{Except.error} なら後続処理へ進まず、\texttt{Except.ok} なら中身を関数 \texttt{f} へ渡す。
\texttt{Option} と同じく、左単位律、右単位律、結合律の三つを確認できる。
...
状態つき処理は、値と更新後の状態を次へ渡す同じ合成形を持つ。
非同期処理も同様に読めるが、本章のモデルはスケジューリング、取消、外部 I/O の意味論までは扱わない。
