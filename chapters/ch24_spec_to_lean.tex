...
この章では、自然言語で書かれた仕様書から、Lean で扱える小さな検証モデルを作る手順を学びます。
目的は、仕様書の全文を機械翻訳することではありません。
入力、出力、状態、操作、事前条件、事後条件、不変条件に分け、証明したい核だけを型、関数、述語、定理として切り出します。
題材には、口座からの出金仕様を使います。
本章で作るモデルは、次章の口座操作ケーススタディへの準備になります。
\end{chapterabstract}
...
\item 証明する性質と証明しない性質を、設計文書上で明示できます。
...
図\ref{fig:ch24-spec-to-lean-pipeline}は、この章で使う分解の流れを示します。
左側には自然言語の仕様があります。
中央では、仕様を入力、状態、操作、条件に分けます。
右側では、それぞれを Lean の型、関数、述語、定理として表します。
下には、証明しない範囲を置きます。
ここを明示することで、「証明済み」という言葉の意味が過大になることを防ぎます。
...
一方、Lean に向いていない性質や、本章ではまだ扱わない性質もあります。
...
これは、Lean でこれらを扱えないという意味ではありません。
扱うには、より大きなモデル、外部システムの仮定、実装との対応付けが必要になります。
本章では、まず小さな核に絞ります。
...
証明した性質だけを書くと、読者はそれ以上の保証があるように誤解することがあります。
したがって、Lean の章や設計文書では、証明しない性質を明示する必要があります。

\begin{warningbox}
「Lean で証明した」と言えるのは、Lean に書いた型、関数、仮定、定理の範囲だけです。
モデルに含めていない外部システム、並行性、永続化、丸め、権限、監査要件は、自動的には保証されません。
\end{warningbox}

本章の口座モデルについて、証明する範囲と証明しない範囲を整理すると、次のようになります。
...
この表は、消極的な注意書きではありません。
Lean の証明を実務で使うための前提です。
...
名詞、操作、条件、成功時、失敗時、対象外の範囲が分かれているためです。
...
中央に Lean で扱う小さなモデルを置き、外側に本章では証明しない実務要素を置きます。
...
この保存性の見方が、後のマイグレーション、関手、自然変換、Pullback の章へつながります。
...
次章では、入金、出金、送金を持つ口座操作の仕様証明へ拡張します。
