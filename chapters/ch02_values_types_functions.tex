...
ここでも新しい概念は小さく区切って導入し、命題や証明にはまだ進まない。
\end{chapterabstract}
...
\FigurePlaceholder{fig-ch02-types-values-functions.png}{0.90}{型・値・関数の関係}{fig:ch02-types-values-functions}
...
基本型と演算子を固定する。
\begin{itemize}
\item \texttt{Nat} の値は \texttt{0}、\texttt{1}、\texttt{2 + 3} のように書く。
\item \texttt{Bool} の値は \texttt{true}、\texttt{false}。\texttt{\&\&} は「かつ」。
\item \texttt{String} の値は \texttt{"lean"} のように二重引用符で囲む。\texttt{++} は連結。
\end{itemize}
\end{syntaxbox}
...
\texttt{Nat}、\texttt{Bool}、\texttt{String} は、それぞれ別の型であり、使える演算子も異なる。

\texttt{def} は、引数のない値も、引数を取る関数も定義できる。

関数の型 \texttt{A -> B} は「\texttt{A} を受け取り \texttt{B} を返す」と読む。
