...
\FigurePlaceholder{fig-ch16-monoid-overview.png}{0.90}{モノイドを「空」と「結合」と「法則」から見る}{fig:ch16-monoid-overview}
...
\FigurePlaceholder{fig-ch16-associativity.png}{0.90}{結合律により括弧の付け方を変えても結果が一致する}{fig:ch16-associativity}
...
\FigurePlaceholder{fig-ch16-config-merge.png}{0.90}{設定マージをモノイドとして扱うときの単位元と結合律}{fig:ch16-config-merge}
...
モノイダル圏のtensorは、対象と射を二変数で組み合わせる構造であり、結合子・単位子とcoherenceを伴う~\cite{riehl-category-theory-in-context}。これは実行時の並列性、独立性、交換可能性を自動的には与えない。
...
\FigurePlaceholder{fig-ch16-monoidal-category.png}{0.90}{逐次合成とテンソルによる並置を区別するモノイダルな見方}{fig:ch16-monoidal-category}
