...
第18章から第21章までは、主に「同じ結果になること」を扱ってきた。リファクタリング前後の式が同じであること、関数がすべての入力で同じ出力を返すこと、adapter が業務ロジックと干渉しないことは、等式で表しやすい。
...
図\ref{fig:ch22-equality-vs-order}は、双方向の一致を要求する等式と、向きを持つ比較を区別している。
情報を落とす変換では round-trip ではなく、保存すべき関係と安全側の順序を選ぶ。
...
Lean では、関係を \texttt{Prop} を返す関数として書き、その成立を定理として証明する。
...
\texttt{publicLevel}、\texttt{internalLevel}、\texttt{secretLevel} を並べる。
...
このコードでは、\texttt{secretLevel}が\texttt{publicLevel}以下であるという仮定を置くと矛盾することを示している。
秘密情報を公開情報としてそのまま扱うのではなく、後で出てくる抽象化やマスキングを通す必要がある、という読み方をする。
...
強い仕様は許す値が少ない。
たとえば「偶数の自然数である」は「自然数である」を含意するので、後者より強い。
...
このコードでは、二つの順序\texttt{leA}と\texttt{leB}を受け取り、関数\texttt{f}が順序を保つことを\texttt{MonotoneMini}として定義している。
入力側で比較できる二つの値が、変換後にも出力側の順序で比較できることを要求している。
...
抽象化は情報を落とす関数であると同時に、どの情報を区別しないかを定める。
本章の \texttt{allow} は、抽象値が許す具体値の最大レベルを返す。
...
図\ref{fig:ch22-abstraction-concretization}では、\texttt{erase} が三段階の詳細度を二段階の公開ビューへまとめ、\texttt{allow} が各ビューの最大詳細度を返す。
...
本章では、\(\alpha(c) \leq a \Longleftrightarrow c \leq \gamma(a)\) の向きを採用する。
前順序を圏と見たとき、これは \(\alpha\) を左随伴、\(\gamma\) を右随伴とする向きである~\cite{riehl-category-theory-in-context}。
...
図\ref{fig:ch22-galois-law}の左右は別々の安全条件ではなく、同じ比較を抽象側と具体側で表した同値な命題である。
...
このコードでは、Galois接続の法則から、具体値を抽象化して戻す方向と、抽象値を具体化して戻す方向の基本的な不等式を取り出している。
\texttt{gc\_unit}と\texttt{gc\_counit}は、抽象化と具体化が互いにどの範囲を覆うかを確認するための一般形である。
...
図\ref{fig:ch22-public-log-mask}は、フィールド保存の関係、レベル順序の単調性、抽象化と具体化の対応を別々の証明義務として示す。
Galois 接続だけでフィールド保存まで自動的に得られるわけではない。
...
\end{itemize}
これらを個別に証明して初めて、「公開ログは詳細ログと同じではないが、定めた仕様に対して安全な近似である」と言える。
...
lossy migration では、完全な round-trip は期待できない。第29章では、この章の考え方を使って、復元ではなく仕様弱化や正規化後の一致として安全性を表す。
