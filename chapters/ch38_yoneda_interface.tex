...
\begin{softwarebox}
インターフェースは、値の内部を直接公開するものではない。むしろ、「利用者がその値に対して何をできるか」を並べたものである。米田補題が扱うのはすべての射と自然性であり、有限個の公開メソッドが常に内部値を決定するという定理ではない。実務では、選んだ観測の範囲で何を同じと見なすかを決めるための見方として使う。
\end{softwarebox}
...
ここで \texttt{recover\_by\_id} は、恒等関数で観測すれば元の値が返ることを示している。\texttt{same\_by\_all\_observers} は、すべての観測関数で結果が同じなら、恒等関数で見ても同じなので、値そのものが等しいことを示している。この二定理から分かるのは、\texttt{x} を \texttt{observeAll x} へ送る対応が値を区別できることまでである。表示した多相関数型だけでは自然性をフィールドとして要求していないため、その型の任意の項が値から生じるという米田同型の逆向きまでは形式化していない。
...
このコードは、一般圏論の自然変換構造を定義してはいない。\texttt{observation\_commutes} が確認するのは、固定した値 \texttt{x} から作った観測族について、先に \texttt{f} で観測して \texttt{g} で後処理することと、合成 \texttt{g} と \texttt{f} を一度に使うことが一致するという自然性の等式である。
...
テストダブルは、本物の実装と同じ内部構造を持つ必要はない。必要なのは、仕様上必要な観測の範囲で同じ振る舞いをすることである。既存テストがたまたま観測する有限個のケースで一致するだけでは、すべての利用文脈で置き換え可能だとは証明できない。
