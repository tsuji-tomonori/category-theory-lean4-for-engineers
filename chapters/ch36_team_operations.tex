...
定理 \texttt{userV1\_rollback\_migrate} は、引用した仕様を \texttt{UserV1} と \texttt{UserV2} の簡略モデル上でそのまま表している。ただし、このモデルの移行は二フィールドをコピーして名前を変えるだけであり、DB I/O、時刻補完、重複ID、パース失敗は扱わない。実際のプロジェクトでは、仕様書の項番、定理名、前提、モデル外の事項をレビュー表で対応づける。
