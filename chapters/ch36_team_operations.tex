...
これらは一人で試すだけでも有用ですが、実務で本当に価値を持つのは、チームの変更プロセスに入ったときです。
...
これらを決めずに導入すると、Lean は品質保証の道具ではなく、読めない追加ファイルになります。
...
証明が壊れたら、変更の意味を確認します。
これを小さく回すのが、Lean をチーム開発に入れる基本形です。
...
\texttt{theorem userV1\_rollback\_migrate} のような名前なら、「UserV1 の rollback と migrate の round-trip についての証明だ」と推測できます。
...
この三つがそろって初めて、Lean の証明はレビュー可能な資料になります。
