...
これらは、次章の等式推論とリファクタリング証明で中心的に使います。
...
この三つが、次章以降の等式推論とリファクタリング証明の基本語彙になります。
\end{leanbox}
...
ここまでで、\texttt{rfl}、\texttt{simp}、\texttt{rw} という等式の道具がそろいました。
次章では、これらを使って、リファクタリングを「意味を保存する書き換え」として証明します。
\texttt{calc} を導入し、段階的な式変形をレビュー可能なログとして書く方法を学びます。
