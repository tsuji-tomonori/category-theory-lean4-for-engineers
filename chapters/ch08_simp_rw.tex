% !TEX root = ../main.tex
\chapter{\texttt{simp}・\texttt{unfold}・\texttt{rw}}
...
これらは、定義の中身を見せたり、式を整理・書き換えたりするための、等式推論の基本道具です。
\end{chapterabstract}
...
本章では、分岐の整理には \texttt{simp}、定義を明示的に展開するには \texttt{unfold}、証明済みの等式による書き換えには \texttt{rw} を導入します。
...
式 & & 展開後 \\
\midrule
\texttt{chargeShipping true shipping} & $\Longrightarrow$ & \texttt{0} \\
\texttt{chargeShipping false shipping} & $\Longrightarrow$ & \texttt{shipping} \\
\bottomrule
\end{tabular}
\end{table}

表\ref{tab:ch08-simp-branches}の矢印は、左の式が定義展開と分岐の評価によって、右の値へ整理されることを表します。
