...
\FigurePlaceholder{fig-ch35-connection-map.png}{0.92}{仕様・実装・テスト・証明の接続地図}{fig:ch35-connection-map}
...
\FigurePlaceholder{fig-ch35-proof-kernel.png}{0.90}{実務仕様から証明可能な核を切り出す}{fig:ch35-proof-kernel}
...
このコードでは、出金可能な場合と不可能な場合を定理として分けている。\path{withdrawCore_success} は、出金額が残高以下なら、結果は残高から出金額を引いた値になることを言う。\path{withdrawCore_failure} は、出金額が残高を超えるなら失敗することを言う。
