...
しかし実務では、次のような判断がすぐに必要になります。

\begin{itemize}
\item この性質を Lean で証明すべきかを判断します。
\item この挙動には単体テストで十分かを判断します。
\item 外部 API の応答や DB の実挙動まで Lean に入れるべきかを判断します。
\item 証明した theorem を、設計文書やレビューでどのように読むかを判断します。
\end{itemize}
