...
本章では、自然変換を「関手どうしを一貫してつなぐ adapter」として導入する。
...
この変換と \texttt{map} の整合性を、自然性という等式で表す。
...
\item 自然性の四角形を「adapter と \texttt{map} で持ち上げた関数の適用順を交換できる」という等式として読める。
...
図\ref{fig:ch15-overview}は、二つの wrapper と、それぞれの \texttt{map}、wrapper 間の adapter の関係を示す。
先に中身を変えても、先に wrapper を変えても結果が一致することが、本章で確かめる自然性である。
...
ここで使う変換は、\texttt{A} が具体的に何であるかを見ない。
\texttt{Nat} でも \texttt{String} でも \texttt{User} でも、規則は同じである。
ただし、実装が payload を調べないことは自然性を理解する助けにはなるが、自然変換の定義そのものではない。
正式な判定条件は、対象ごとの成分を持ち、定義域の圏のすべての射について \texttt{map} と可換になることである。

\subsection{よくある危険：値の中身を勝手に見る adapter}

本章の \texttt{Option} から \texttt{List} への adapter は、中身の型 \texttt{A} の具体的な性質に依存しない。
...
このような混入は自然性を壊すことがあるため、可換性の証明を設計上の検査に使える。
ただし、何を射として許すかを制限した別の圏では自然性の条件も変わる。

\section{自然変換を定義する}

自然変換の標準定義では、二つの関手の各対象に成分を与え、定義域の圏の任意の射について自然性の四角形を可換にする~\cite{riehl-category-theory-in-context}。
...
Lean では、自然性の式を直接読めるように、\texttt{optionMap} と \texttt{optionToList} を小さく自作する。
...
したがって、\texttt{optionToList} については、\texttt{Option} 内で \texttt{f} を適用してから \texttt{List} へ移す処理と、先に \texttt{List} へ移してから \texttt{f} を適用する処理を置き換えられる。
...
少なくとも本章のように \texttt{Type} のすべての関数を射として扱うなら、自然性はそのような型固有の処理を許さない強い条件になる。
...
「adapter は箱の形を変え、\texttt{map} される処理とは干渉しない」という説明は、この等式の実務的な読み替えである。
形式的な根拠は、あくまで選んだ関手と射に対して自然性の等式が成り立つことである。
...
図\ref{fig:ch15-api-adapter}では、上段が旧 wrapper 内で変換してから adapter を適用する経路、下段が先に新 wrapper へ移してから変換する経路である。
次のモデルでは、この二経路の一致を定理にする。
...
自然性の証明から直接分かるのは、adapter が指定した \texttt{map} と可換であることまでである。
本章のように wrapper の構造変換へ責務を絞ると、その条件を満たす設計にしやすい。
その結果、次のような判断もしやすくなる。
...
ただし、これらが自然でないという結論は、本章と同じく全型と全関数を対象にした場合の話である。
型クラスを備えた対象と、その構造を保つ射だけに範囲を変えれば、対応する自然性は別途判定し直す必要がある。

\section{本章のまとめ}

自然変換は、二つの関手の各対象に成分を持ち、すべての射について自然性を満たす変換である。
...
API response wrapper の変更では、adapter と \texttt{map} が可換であることを示すと、モデル化した変換の範囲で両者の適用順を交換できると説明できる。
