...
\FigurePlaceholder{fig-ch18-rewriting-flow.png}{0.92}{等式証明を、変更前の式から正規形へ進む変形ログとして読む}{fig:ch18-rewriting-flow}
...
\begin{syntaxbox}
\texttt{@[simp]} は、直後の定理や定義に属性を付ける構文である。
\begin{itemize}
\item \texttt{@[...]} は属性指定で、Lean に追加の扱い方を伝える。
\item \texttt{@[simp] theorem ...} は、その定理を \texttt{simp} が使う単純化規則として登録する。
\item 属性は証明そのものではなく、「この証明済みの事実をどの自動化で使うか」という設定である。
\end{itemize}
\end{syntaxbox}
...
\FigurePlaceholder{fig-ch18-price-normalization.png}{0.90}{価格計算ロジックを同じ正規形へ寄せて比較する}{fig:ch18-price-normalization}
