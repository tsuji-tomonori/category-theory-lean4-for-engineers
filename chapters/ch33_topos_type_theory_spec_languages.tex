...
\label{chap:ch33-topos-type-theory-spec-languages}
...
第31章では、随伴を「自由に作ること」と「構造を忘れること」の対応として眺めた。第32章では、米田の補題を「対象は、それに対する観測方法によって特徴づけられる」という直感として扱った。本章では、その先にあるトポス、型理論、仕様言語への入口を作る。
...
\FigurePlaceholder{fig-ch33-bridge-map.png}{0.92}{トポス、型理論、仕様言語への橋渡し。中央に「世界の中で論理を語る」という視点を置き、左に圏論、右にLeanと型理論、下に仕様DSLとモデル検査を配置する。}{fig:ch33-bridge-map}
...
\FigurePlaceholder{fig-ch33-internal-language.png}{0.90}{外部から見た世界と内部言語。外側には複数の文脈があり、それぞれの内側に、その文脈でだけ使える論理や仕様があることを示す。}{fig:ch33-internal-language}
...
\FigurePlaceholder{fig-ch33-spec-dsl-loop.png}{0.92}{仕様DSLの構文、意味論、検査の関係。DSL式を状態空間上の命題へ解釈し、Leanの証明またはモデル検査で確認する流れを示す。}{fig:ch33-spec-dsl-loop}
