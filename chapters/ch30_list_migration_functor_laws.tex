...
第28章では、\texttt{UserV1} と \texttt{UserV2} の間の lossless migration を、1 件のデータに対して扱いました。
...
第28章で扱った lossless migration では、1 件について次の二つを証明しました。
...
DB 更新 & 本章では直接扱わない & トランザクション、ロック、リトライ、部分失敗は実装、運用側の検査が必要です。 \\
\bottomrule
\end{longtable}
...
一方、本章では、失敗する移行、外部副作用、DB 更新、並び順を保証しない処理、重複除去を伴う処理は扱いませんでした。
それらを扱うには、\texttt{Option}、\texttt{Except}、状態、関係、集合的な仕様などを導入する必要があります。
本章の目的は、まず純粋なリスト変換の核を正確に理解することです。
