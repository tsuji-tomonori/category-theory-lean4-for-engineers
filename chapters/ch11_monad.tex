...
\FigurePlaceholder{fig-ch11-overview.png}{0.90}{モナドを文脈つき処理の合成として見る}{fig:ch11-overview}
...
\label{lst:ch11_option_bind}
\end{listing}

コード \ref{lst:ch11_option_bind} の一つ目の \texttt{\#eval} は、\texttt{some 10} の中身を取り出して \texttt{some 11} を作る。
二つ目は、最初から \texttt{none} なので、後続の関数を実行せずに \texttt{none} になる。
この「失敗を自動的に伝播する」振る舞いが、バリデーションやマイグレーションで重要になる。

\FigurePlaceholder{fig-ch11-bind-flow.png}{0.90}{\texttt{bind} が成功時だけ次の処理へ進む流れ}{fig:ch11-bind-flow}
...
\FigurePlaceholder{fig-ch11-monad-laws.png}{0.92}{モナド則を effectful code のリファクタリング規則として読む}{fig:ch11-monad-laws}
...
\FigurePlaceholder{fig-ch11-migration-pipeline.png}{0.92}{失敗しうる移行ステップを \texttt{bind} で合成する}{fig:ch11-migration-pipeline}
...
\begin{summarybox}
\begin{itemize}
\item \texttt{Option A}、\texttt{Except E A}、\texttt{S -> (A, S)} は、いずれも文脈を持つ値として読める。
\item \texttt{pure} は普通の値を文脈に入れ、\texttt{bind} は文脈つきの値を次の文脈つき処理へつなぐ。
\item モナド則は、effectful code の左単位、右単位、結合律というリファクタリング規則として読める。
\item Lean では、\texttt{Option} の \texttt{none} と \texttt{some} を場合分けすることで、モナド則を小さく証明できる。
\item 失敗しうるマイグレーションでは、失敗可能性を返り値の型に入れ、\texttt{bind} で段階的に合成すると、失敗ケースを漏らしにくくなる。
\item 失敗理由が必要な場合は \texttt{Option} ではなく \texttt{Except} を使う。
\item 状態つき処理や非同期処理も、値だけでなく文脈を次へ渡す合成として読むことができる。
\end{itemize}
\end{summarybox}
