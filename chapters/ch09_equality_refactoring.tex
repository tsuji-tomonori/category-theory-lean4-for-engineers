...
本章では、Lean で大きなプログラム全体を検証することを目的にしません。
...
変更前後で同じ結果を返すという主張は、自然言語のレビューコメントとしても書けます。
...
戻り値の等しさは、純粋な関数を小さくモデル化するときに扱いやすい性質です。
...
\texttt{namespace Chapter09} は、本章の名前を他章と衝突させないための囲いです。
...
結合律により、\texttt{shipping} と \texttt{handling} を先にまとめても合計が変わらないことを確認しています。
