...
\begin{itemize}
\item 第18章の等式推論では、式や計算結果の一致を扱いました。
\item 第19章の場合分けでは、入力の形ごとに証明を分けました。
\item 第20章の帰納法では、任意の長さのリストや再帰構造を扱いました。
...
本章では、関数外延性を「すべての入力で同じ出力を返すなら、関数として同じと見てよい」という道具として扱いました。
これは圏論上、射の等しさに関わる基本的な見方であり、Lean では \texttt{funext} という具体的な証明手順として現れます。

本章で扱ったのは、主に純粋な関数の戻り値の等しさです。
より現実的な wrapper では、ログ、状態、エラー処理をどうモデル化するかが問題になります。
その場合も基本方針は同じです。
観測したいものを型に入れ、任意の入力について結果が一致することを示します。

後続の章では、完全な等しさだけでは扱いにくい場面へ進みます。
第22章では、等しいとは言えなくても「より詳しい」「より安全」「より抽象的」といった比較を、関係や前順序として扱います。
