...
このコードでは、\texttt{migrateUser u}で作った新形式の値から、\texttt{.userId}でフィールドを取り出している。
構造体の変換を読むときは、どのフィールドがどの名前へ移ったかを、このような小さな関数で確認するとよい。
...
このコードでは、ゴールが二つの条件の組になっているため、\texttt{constructor}で左右のゴールへ分けている。
それぞれのゴールは定義どおりの等式なので、手元の証拠として\texttt{rfl}を\texttt{exact}に渡して閉じている。
...
このコードでは、仮定\texttt{hp : P}がそのままゴール\texttt{P}の証拠になっている。
\texttt{exact hp}は、新しい証明を作るのではなく、すでに手元にある証拠を提出する読み方をする。
...
\texttt{apply h} は、「\texttt{Q} を示すには \texttt{h : P -> Q} が使えるので、残る前提 \texttt{P} を示せばよい」とゴールを変える。\texttt{exact} が完成済みの証拠を渡すのに対し、\texttt{apply} は使う証明の形を先に決める。
