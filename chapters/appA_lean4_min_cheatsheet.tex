...
\begin{syntaxbox}
\texttt{apply} は、現在のゴールに合う定理や関数を適用して、必要な前提を新しいゴールとして出す構文である。
\begin{itemize}
\item \texttt{apply h} は「このゴールは \texttt{h} を使えば示せるはずなので、\texttt{h} に必要な材料をそろえる」と読む。
\item \texttt{exact} が完成済みの証明を渡すのに対し、\texttt{apply} は証明の形を先に決めて残りを埋める。
\end{itemize}
\end{syntaxbox}
