...
\FigurePlaceholder{fig-ch25-account-flow.png}{0.92}{入金・出金・送金を状態遷移として見る}{fig:ch25-account-flow}
...
\FigurePlaceholder{fig-ch25-withdraw-transfer.png}{0.92}{出金の成功・失敗と送金の合成}{fig:ch25-withdraw-transfer}
...
\FigurePlaceholder{fig-ch25-total-preservation.png}{0.92}{送金成功時に総残高が保存される理由}{fig:ch25-total-preservation}
...
\begin{syntaxbox}
\texttt{by\_cases} は、命題が成り立つ場合と成り立たない場合に分ける構文である。
\begin{itemize}
\item \texttt{by\_cases h : P} は、\texttt{P} が真の場合と偽の場合の二つのゴールに分ける。
\item 真の場合の枝では、\texttt{h : P} が仮定として使える。
\item 偽の場合の枝では、\texttt{h : ¬ P} が仮定として使える。
\item 本文の例では、\texttt{amount <= src.balance} が成り立つかどうかで、送金成功側と失敗側を分けている。
\end{itemize}
\end{syntaxbox}
