...
本章は、本書全体の地図です。
圏論や Lean 4 の細部に入る前に、ソフトウェア開発で何を「正しい」と呼ぶのか、変更前後で何を保存したいのか、その主張をどのように検査可能な形へ落とすのかを整理します。
テスト、型、仕様、証明は、互いに競合する道具ではありません。
それぞれが異なる粒度で正しさを支えます。
本章では、圏論を合成と変換の言語として、Lean 4 を仕様を小さく検査する実験室として位置づけます。
\end{chapterabstract}
...
\begin{warningbox}
「テストがあるから仕様は不要」「型があるから証明は不要」「証明したから実装全体が安全」という言い方は、いずれも保証範囲を越えています。
...
Lean で小さな \texttt{UserV1} と \texttt{UserV2} の round-trip を証明しても、本番のシリアライザ、DB 制約、文字コード、外部 API の全挙動まで自動的に保証されるわけではありません。
...
Lean は厳密な道具ですが、モデル化した範囲を越えることは証明しません。
...
本書では、最初から現実の JSON パーサを完全にはモデル化しません。
まず、何を同じと見なすかを仕様として切り出します。
...
この性質は、後の章で自然性として扱います。
自然性は抽象的な言葉ですが、実務的には「どの型でも一貫して動く adapter の条件」と読めます。
...
\begin{enumerate}
\item 第1章から第10章で、Lean 4 における型、関数、命題、不変条件を学びます。
\item 第11章から第15章で、圏、同型、積・余積、関手、自然変換をソフトウェアの例で理解します。
\item 第24章から第31章で、仕様証明、マイグレーション、API adapter のケーススタディを確認します。
...
\begin{enumerate}
\item 第1章から第10章で Lean の最小構文を学びます。
\item 第18章から第21章で、等式、場合分け、帰納法、関数外延性を学びます。
\item 第24章から第26章で、自然言語仕様を Lean の定理へ落とす流れを確認します。
\end{enumerate}
...
圏論の概念そのものを丁寧に身につけたい場合は、第11章から第17章をゆっくり読みます。
対象、射、合成、同型、関手、自然変換、モノイド、モナドを、すべてソフトウェアの例と結びつけて理解します。
その後、第22章、第23章、第37章以降へ進むと、より抽象的な概念へ移りやすくなります。

\begin{warningbox}
どのルートでも、最初から Mathlib の高度な \texttt{CategoryTheory} API に進む必要はありません。
本書では、まず自作の小さな定義を使います。
概念を理解してから既存ライブラリへ進むことで、Lean と圏論を同時に学ぶ負荷を減らせます。
\end{warningbox}

\section{この章でまだ扱わないこと}
\label{sec:ch00-out-of-scope}

本章は地図であり、詳細な証明を扱う章ではありません。
したがって、次の内容は後続章で扱います。

\begin{itemize}
\item Lean 4 の基本構文と、\texttt{def}、\texttt{theorem}、\texttt{by}、\texttt{simp}、\texttt{rw} の使い方
...
\begin{intuitionbox}
この章で確認する問いは、「この変更で、何が保存されるべきなのか」です。
この問いに答えられるようになると、テストを書くべき箇所、型で表すべき制約、仕様として明文化すべき性質、Lean で証明すべき核が見え始めます。
\end{intuitionbox}
