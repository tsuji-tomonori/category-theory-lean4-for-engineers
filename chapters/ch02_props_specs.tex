...
\begin{syntaxbox}
\texttt{Prop} は、証明したい主張を置くための種類である。
\begin{itemize}
\item \texttt{Bool} は実行時に \texttt{true} または \texttt{false} へ計算される値である。
\item \texttt{Prop} は「この条件が成り立つ」という命題を表す。
\item Lean では、仕様を \texttt{Prop} として書き、その \texttt{Prop} を証明する。
\end{itemize}
\end{syntaxbox}
...
\begin{syntaxbox}
\texttt{example} は、名前を付けない小さな定理である。
\begin{itemize}
\item \texttt{example : P := ...} は「命題 \texttt{P} の証明例を書く」と読む。
\item 後から再利用する必要がある主張には \texttt{theorem} を使い、説明用の短い確認には \texttt{example} を使う。
\end{itemize}
\end{syntaxbox}
...
\begin{syntaxbox}
\texttt{simp} は、Lean による自動的な単純化である。
\begin{itemize}
\item 定義を展開したり、明らかな等式を整理したりする。
\item \texttt{simp [name]} のように角括弧へ名前を書くと、その定義や補題も使って単純化する。
...
\begin{syntaxbox}
\texttt{rw} は、等式を使って式を書き換える構文である。
\begin{itemize}
\item \texttt{rw [h]} は、等式 \texttt{h} を使って現在のゴールを書き換える。
\item \texttt{rw [<- h]} のように左向き矢印を付けると、等式を逆向きに使う。
\item リファクタリングの説明では「同じ意味の式へ置換する」操作として読む。
\end{itemize}
\end{syntaxbox}
...
\begin{syntaxbox}
全称量化と導入の構文が初めてまとまって出てくる。
\begin{itemize}
\item \texttt{∀ x, P x} は「任意の \texttt{x} について \texttt{P x} が成り立つ」と読む。
\item \texttt{forall x, P x} も \texttt{∀ x, P x} と同じ全称量化である。本書では両方を見かける。
\item \texttt{intro x} は、任意の値 \texttt{x} を仮に受け取って証明を始める操作である。
\item 複数の値や仮定をまとめて受け取るときは \texttt{intros} が使われる。
\end{itemize}
\end{syntaxbox}
...
\texttt{Bool} は実行時に計算される真偽値であり、\texttt{Prop} は証明したい命題である。

仕様は、関数の外側に命題として切り出すと Lean で検査できる。

\texttt{example} はその場限りの確認、\texttt{theorem} は名前付きで再利用できる仕様である。

\texttt{by} は証明を書く入口であり、\texttt{rfl}、\texttt{simp}、\texttt{rw} は初期章で使う基本的な証明道具である。

税込金額計算や DTO 変換の ID 保存のような小さな性質は、Lean の命題と証明として表せる。
