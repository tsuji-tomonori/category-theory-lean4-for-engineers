...
\begin{summarybox}
\begin{itemize}
\item lossy migration は完全同型ではない。二つの変換関数があっても、round-trip law がなければ安全な相互変換とは言えない。
\item 強い仕様が成り立たないときは、反例を明示する。これにより、保証できないことを設計レビューで隠さず扱える。
\item 仕様弱化では、完全復元ではなく、表示互換、検索キー保存、集計結果保存など、業務上必要な観測を選んで証明する。
\item well-formed 条件を置けば、入力範囲を狭めた条件付き round-trip を証明できる場合がある。
\item lossy migration は正規化として見ることもできる。一度正規化した値が安定することは、実務上有用な保証になる。
\end{itemize}
\end{summarybox}
