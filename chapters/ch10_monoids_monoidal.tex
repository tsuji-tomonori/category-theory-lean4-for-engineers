...
最後に、モノイダル圏を「逐次合成だけでなく、並列合成も扱うための言語」として軽く眺める。
\end{chapterabstract}
...
\item モノイダル圏を、並列合成や資源合成を考えるための発展的な見方として位置づけられる。
\end{learninggoals}
...
\begin{intuitionbox}
モノイドは、「空」と「結合」に対する最低限の約束である。
空の値は初期値になる。
結合演算は、二つの結果を一つにまとめる。
結合律があれば、三つ以上をまとめるときに括弧の付け方を気にしなくてよくなる。
ただし、順序を入れ替えてよいとは限らない。
\end{intuitionbox}

\FigurePlaceholder{fig-ch10-monoid-overview.png}{0.90}{モノイドを「空」と「結合」と「法則」から見る}{fig:ch10-monoid-overview}
...
\begin{softwarebox}
ログ集約では、単体のログ値よりも「どう結合するか」が重要になる。
空のログは \texttt{[]} であり、二つのログはリスト連結でまとめられる。
このとき、空ログを前後に足しても結果が変わらないなら、初期値として安心して使える。
\end{softwarebox}
...
\begin{intuitionbox}
モノイドの発想は、「処理結果をあとからまとめられる形にしておく」ことである。
各処理が部分結果を返し、それらを結合すれば全体結果になる。
この形にしておくと、分割処理、バッチ処理、ストリーム処理、並列処理の設計がしやすくなる。
\end{intuitionbox}
...
\FigurePlaceholder{fig-ch10-associativity.png}{0.90}{結合律により括弧の付け方を変えても結果が一致する}{fig:ch10-associativity}
...
\begin{softwarebox}
単位元は、集約処理における自然な初期値である。
空のリストを集約しても値が返せるため、空ケースだけを特別な分岐として扱う必要が減る。
このことは、実装を単純にするだけでなく、仕様も読みやすくする。
\end{softwarebox}
...
\begin{leanbox}
Lean では、モノイド法則を「すべての値について成り立つ等式」として書く。
ログ結合なら、\texttt{[]} が単位元であり、\texttt{++} が結合演算である。
結合律と単位元の法則を定理として書くことで、ログ集約の括弧を変えても安全であることを確認できる。
\end{leanbox}
...
\end{minted}
\caption{ログ結合の単位元と結合律}
\label{lst:ch10_log_laws}
\end{listing}
...
\end{minted}
\caption{学習用の小さなモノイド構造}
\label{lst:ch10_simple_monoid}
\end{listing}
...
\begin{softwarebox}
結合律があると、集約木の形を変えられる。
たとえば、三つのログチャンク \texttt{a}, \texttt{b}, \texttt{c} をまとめるとき、先に \texttt{a} と \texttt{b} をまとめても、先に \texttt{b} と \texttt{c} をまとめても、順序を保っている限り最終結果は同じになる。
\end{softwarebox}
...
\end{minted}
\caption{三つのログチャンクの集約計画は括弧に依存しない}
\label{lst:ch10_log_plan}
\end{listing}
...
theorem mergeConfig_empty_right (c : Config) :
  ...
  simp [mergeConfig, emptyConfig]
\end{minted}
\caption{設定マージの小さなモデル}
\label{lst:ch10_config_merge}
\end{listing}
...
\end{minted}
\caption{設定マージの結合律}
\label{lst:ch10_config_assoc}
\end{listing}

この小さなモデルでは、設定マージは結合律を満たす。
そのため、\texttt{(base <> env) <> service} と \texttt{base <> (env <> service)} は同じ意味になる。

\FigurePlaceholder{fig-ch10-config-merge.png}{0.90}{設定マージをモノイドとして扱うときの単位元と結合律}{fig:ch10-config-merge}
...
\begin{intuitionbox}
普通の圏では、射を縦に合成する。
たとえば \texttt{A -> B} と \texttt{B -> C} をつないで、\texttt{A -> C} を作る。
モノイダル圏では、これに加えて、射を横に並べる操作も考える。
たとえば \texttt{A -> B} と \texttt{C -> D} を並べて、\texttt{(A, C) -> (B, D)} のような処理として見る。
\end{intuitionbox}

ソフトウェアでは、次のような場面でこの見方が役に立つ。

\begin{itemize}
\item 二つの独立した入力を別々に検証し、結果をペアとしてまとめる。
\item 複数のサービス呼び出しを独立に実行し、最後に結果を結合する。
\item 画像処理やETLで、別々の列やチャネルを並列に変換する。
\item リソースや権限を合成し、全体として使える処理を作る。
\end{itemize}

\FigurePlaceholder{fig-ch10-monoidal-category.png}{0.90}{逐次合成と並列合成を区別するモノイダルな見方}{fig:ch10-monoidal-category}
...
\begin{summarybox}
\begin{itemize}
\item モノイドは、単位元と結合演算を持ち、単位元の法則と結合律を満たす構造である。
\item ログ、リスト、文字列、合計、設定マージなど、多くの実務上の集約処理はモノイド的に設計できる。
\item 結合律は、括弧の付け方を変えてもよいことを保証するが、順序を入れ替えてよいことは保証しない。
\item 単位元は、空入力や初期値を自然に扱うための仕様である。
\item Lean 4 では、モノイド法則を等式として書き、ログ集約や設定マージの小さな安全性を確認できる。
\item モノイダル圏は、逐次合成に加えて並列合成を扱う発展的な言語として理解できる。
\end{itemize}
\end{summarybox}
