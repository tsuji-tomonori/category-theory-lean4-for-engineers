...
これは、第12章で扱った「同型を構造体として持つ」考え方の実務版です。
...
本章では、すべての情報が一対一に対応する移行を扱いました。
次章では、この前提が崩れます。
たとえば、\texttt{fullName} を \texttt{firstName} と \texttt{lastName} に分割する場合、入力によっては元の形へ完全には戻せません。
そのときは、無理に同型として扱うのではなく、well-formed 条件や正規化後の一致へ仕様を弱める必要があります。
