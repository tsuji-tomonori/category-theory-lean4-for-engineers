...
\section{\texttt{rw}、\texttt{simp}、\texttt{calc} の使い分け}
...
\begin{summarybox}
\begin{itemize}
\item リファクタリングや計算式の整理は、多くの場合「変更前後が等しい」という主張として表せる。
\item \texttt{rw} は、検査済みの等式を使う根拠付きの置換である。
\item \texttt{simp} は、不要なノイズを消して式を正規形へ寄せる道具である。
\item \texttt{calc} は、途中式と変形理由を残すレビュー可能な証明ログである。
\item 正規化を意識すると、変更前後のロジックを比較しやすくなる。
\item Lean で証明した範囲と、実務コード側に残る前提は必ず分けて書く。
\end{itemize}
\end{summarybox}
