...
これは、第10章で扱った「条件を満たす値」を使う考え方の応用です。
...
これは、第30章で扱った「一件の移行をリスト全体へ持ち上げる」考え方と同じです。
...
一方、この定理は多くのことを保証していません。
ここを曖昧にすると、Lean の証明を過大評価してしまいます。

\begin{warningbox}
本章の Lean モデルは、次の性質を証明していません。
\begin{itemize}
\item SQL の実際の実行計画が正しいことです。
\item インデックス、ロック、トランザクション分離レベルが期待どおりであることです。
\item NULL、重複行、照合順序、タイムゾーン、文字コードを含む DB エンジン固有の挙動です。
\item 本番データの全件が移行スクリプトの前提を満たしていることです。
\item 移行処理が途中で失敗したときの復旧手順です。
\item 新スキーマ上で新たに作られる任意のデータが旧クエリと互換であることです。
\item 同じ \texttt{userId} を持つ重複ユーザー、孤児注文、同一 key で異なる名前を持つ行を join したときの件数と選択規則です。
\item SQL の bag semantics における重複数や、結果順序の保存です。
\end{itemize}
\end{warningbox}
...
これは弱点ではなく、境界線です。
Lean モデルは、仕様の中核を小さく切り出すことで力を発揮します。
逆に、実 DB 全体の挙動をすべて一つの Lean ファイルで表そうとすると、学習目的にも実務目的にも重くなりすぎます。
