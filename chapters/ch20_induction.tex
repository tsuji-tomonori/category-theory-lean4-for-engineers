...
しかし本番の移行対象は1件とは限らず、数千件、数百万件、または件数が事前には分からないリストです。
...
本章では、リストと木について次の性質を証明しました。

\begin{itemize}
\item \texttt{List.map} は長さを保存します。
\item ユーザー一覧の移行は件数を保存します。
\item ユーザー一覧の移行はID列を保存します。
\item \texttt{append} 後の長さは、左右の長さの和になります。
\item \texttt{Tree.map} はノード数を保存します。
\item ユーザー木の移行はノード数を保存します。
\end{itemize}

これらは、すべて「任意の大きさのデータ構造」に対する主張です。
空、1件、2件、3件だけを確認したわけではありません。
Lean の \texttt{induction} によって、すべての形を覆う証明になっています。

一方、本章では次のことは証明していません。
...
この境界を明示することは、形式化を実務で使ううえで重要です。
証明は強い道具ですが、証明した範囲を越えて主張してはいけません。
...
\texttt{append} の長さに関する性質は、分割バッチや結合処理の説明に使えます。

木構造では、左右の部分木に対する帰納仮定を使って、木全体の性質を証明します。

本章の証明は、第30章の \texttt{List} 全体のマイグレーションと関手則へ直接つながります。

次章では、関数そのものが等しいとはどういうことかを扱います。
リストや木の中身を変換するだけでなく、2つの adapter や wrapper が同じ振る舞いをすることを示すには、すべての入力で結果が一致するという見方が必要です。
