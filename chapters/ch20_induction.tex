...
確認したいのは、任意のユーザー一覧 \texttt{xs} について
\texttt{xs.map migrateUser} の件数と ID 列が保存されることである。
...
図\ref{fig:ch20-list-induction}では、空リストから始めて、先頭に1つずつ要素を追加していく様子を表している。
...
\texttt{(xs ++ ys).length = xs.length + ys.length} と表せる。
...
\texttt{map\_preserves\_length} を \texttt{migrateUser} へ適用した証明を渡している。
証明そのものは短い。
...
木に対する帰納法では、葉のケースと、左右の部分木を持つノードのケースを扱う。
後者では左右それぞれについて帰納仮定を使える。

\FigurePlaceholder{fig-ch20-tree-induction.png}{0.90}{木の構造帰納法}{fig:ch20-tree-induction}

図\ref{fig:ch20-tree-induction}では、1つのノードが左部分木と右部分木を持つ様子を表している。
...
\texttt{tree\_map\_}\allowbreak\texttt{preserves\_size} は、任意の型と任意の変換関数について成り立つ。
\texttt{migrateUserTree\_}\allowbreak\texttt{preserves\_size} は、それをユーザー移行へ特殊化した定理である。
...
帰納法は、任意の長さのリストや任意の深さの木に対する仕様を扱うための道具である。

リストでは、空リストのケースと、先頭要素を追加する \texttt{cons} ケースを確認する。

\texttt{List.map} の件数保存は、バッチ移行の基本仕様として使える。

\texttt{append} の長さに関する性質は、分割バッチや結合処理の説明に使える。

木構造では、左右の部分木に対する帰納仮定を使って、木全体の性質を証明する。

本章の証明は、第30章の \texttt{List} 全体のマイグレーションと関手則へ直接つながる。
