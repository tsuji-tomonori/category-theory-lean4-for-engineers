...
\FigurePlaceholder{fig-ch27-overview.png}{0.90}{合成可能な設計の全体像}{fig:ch27-overview}
...
\FigurePlaceholder{fig-ch27-boundaries.png}{0.90}{入出力型で処理境界をそろえる}{fig:ch27-boundaries}

ここで重要なのは、型を細かくしすぎることではない。重要なのは、意味が変わる境界に型名を与えることである。たとえば、単なる整形処理で意味が変わらないなら、型を分ける必要はないかもしれない。一方、生データから検証済みデータへ移る境界は、仕様上の意味が大きく変わるので、型を分ける価値がある。
...
\begin{leanbox}
ここでは関数そのものの等しさではなく、「任意の入力 \texttt{x} に対して結果が同じ」という形で書く。これは第15章の関数外延性に進む前でも読みやすい形である。
\end{leanbox}
...
\end{minted}
\caption{関数合成の恒等律と結合律}
\label{lst:ch27-composition-laws}
\end{listing}

この証明が短いのは、Leanにとって関数合成の両辺が定義を展開すると同じ形になるからである。これは実務で「どんな処理でも順序を変えてよい」という意味ではない。ここで証明しているのは、純粋な関数合成の括弧の付け方だけである。副作用や失敗を含む処理については、\texttt{Option}、\texttt{Except}、状態などの文脈を型に出す必要がある。
...
\FigurePlaceholder{fig-ch27-etl-pipeline.png}{0.92}{ETLパイプラインを射の合成として見る}{fig:ch27-etl-pipeline}

最初に、段階ごとの型を定義する。ここでは説明を短くするため、金額のパースや日付処理は省略し、\texttt{Nat}だけで扱う。
...
structure ReportRow where
  ...
  deriving Repr, DecidableEq
\end{minted}
\caption{ETLの段階を型として分ける}
\label{lst:ch27-etl-types}
\end{listing}

この段階では、まだ圏論らしい抽象は出てこない。ただし、型を分けたことで、\texttt{RawRow -> ValidRow}、\texttt{ValidRow -> EnrichedRow}、\texttt{EnrichedRow -> ReportRow} という射の列が見えるようになる。
...
\end{minted}
\caption{ETLステージを関数として定義する}
\label{lst:ch27-etl-functions}
\end{listing}

\texttt{pipeline} は、三つの処理を合成したものである。\texttt{pipelineExpanded} は、同じ処理を入れ子の関数適用として書いたものである。設計レビューでは、どちらの見た目で読むかが変わることはあっても、意味は同じであってほしい。
...
\end{minted}
\caption{合成したパイプラインと展開した処理が一致する}
\label{lst:ch27-etl-proofs}
\end{listing}
...
\end{minted}
\caption{ステージ合成の右恒等律と結合律}
\label{lst:ch27-stage-laws}
\end{listing}

\begin{leanbox}
\texttt{Stage} は本格的な圏論ライブラリではない。あくまで、設計上の「処理ステージ」を小さくモデル化するための構造体である。この程度のモデルでも、恒等処理や結合の確認をLeanに任せられる。
\end{leanbox}
...
\begin{warningbox}
副作用を含む処理でも、何らかの形で合成できることは多い。ただし、その場合の法則は単純な関数合成の法則とは限らない。失敗、状態、非同期、ログなどを含む処理は、第11章のモナドや第28章のeffectful codeのリファクタリング規則と合わせて考える必要がある。
\end{warningbox}
...
\begin{summarybox}
\begin{itemize}
\item 合成可能な設計では、部品そのものだけでなく、部品の接続可能性を見る。
\item 入出力型を明示すると、処理ステージの境界と意味がレビューしやすくなる。
\item 恒等律は、何もしない処理を挿入しても意味が変わらないこととして読める。
\item 結合律は、パイプラインの括り方を変えても純粋な変換結果が変わらないこととして読める。
\item Leanでは、ETLのような実務例から純粋な変換の核を取り出し、小さな等式として検査できる。
\item 副作用、失敗、外部システムは、本章の純粋なモデルとは分けて扱う必要がある。
\end{itemize}
\end{summarybox}
