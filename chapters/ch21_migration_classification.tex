...
\label{chap:ch21-migration-classification}
...
\begin{intuitionbox}
マイグレーションを安全に扱う第一歩は、「移せるか」ではなく「何が保存されるか」と問うことである。保存されるものが完全な情報なら同型に近い。保存されるものが一部の性質だけなら、仕様を弱めている。保存されるものが旧データの読み込み可能性だけなら、片方向互換である。
\end{intuitionbox}

本章の目的は、マイグレーションを一律に安全・危険と判定することではない。どの種類の安全性を主張しているのかを分類し、それぞれに対応する証明義務を明確にすることである。

\FigurePlaceholder{fig-ch21-migration-taxonomy.png}{0.92}{マイグレーションを、完全同型、片方向互換、情報損失、制約付き同型、仕様弱化へ分類する概念地図}{fig:ch21-migration-taxonomy}
...
\begin{softwarebox}
\texttt{migrate} は「移行スクリプト」や「adapter」に対応する。\texttt{rollback} は「逆向きの変換」や「旧API互換層」に対応する。ただし、関数が存在するだけでは不十分である。証明したいのは、関数を通した結果が期待した性質を保存することである。
\end{softwarebox}
...
\end{minted}
\caption{完全同型を表す最小構造}
\label{lst:ch21-equivlike}
\end{listing}

\begin{leanbox}
\texttt{EquivLike A B} は、\texttt{A} と \texttt{B} の間に情報を失わない相互変換があることを表すミニ定義である。\texttt{toFun} が移行、\texttt{invFun} が逆変換、\texttt{left\_inv} と \texttt{right\_inv} が二つの round-trip 証明である。
\end{leanbox}
...
theorem user_right_roundtrip (v : UserV2) :
  ...
  rfl
\end{minted}
\caption{フィールド名変更は完全同型として証明できる}
\label{lst:ch21-user-roundtrip}
\end{listing}

この例で Lean が確認しているのは、個別のテストデータではない。任意の \texttt{UserV1} と任意の \texttt{UserV2} について、変換して戻すと元の値に戻る、という一般的な主張である。

\FigurePlaceholder{fig-ch21-roundtrip-laws.png}{0.88}{完全同型では、旧形式から新形式へ移して戻す経路と、新形式から旧形式へ戻して移す経路の両方が恒等変換になる}{fig:ch21-roundtrip-laws}
...
theorem old_roundtrip (o : OldToken) :
  ...
  rfl
\end{minted}
\caption{フィールド追加による片方向互換}
\label{lst:ch21-one-way}
\end{listing}
...
theorem different_users_can_have_same_public_view (msg : String) :
  ...
  rfl
\end{minted}
\caption{匿名化は情報損失を伴うが、保存される性質は証明できる}
\label{lst:ch21-lossy-log}
\end{listing}

最後の定理は、異なる \texttt{userId} を持つ二つの内部ログが、同じ公開ログになりうることを示している。これは、匿名化が同型ではないことを理解するための小さな証拠である。

\begin{intuitionbox}
情報損失ありの移行では、「元に戻る」ではなく、「何が保存されるか」を証明する。匿名化なら、公開してよい情報だけが残ること、秘匿したい情報が観測できないこと、表示上必要なメッセージが保存されることなどが証明候補になる。
\end{intuitionbox}
...
theorem new_roundtrip_when_default
  ...
  | mk token issuedAt =>
      ...
      rfl
\end{minted}
\caption{well-formed 条件を仮定した round-trip}
\label{lst:ch21-conditional-roundtrip}
\end{listing}
...
\FigurePlaceholder{fig-ch21-proof-choice.png}{0.90}{マイグレーションで証明すべき性質を選ぶための判断フロー}{fig:ch21-proof-choice}

表の見方は単純である。まず、移行が完全復元を要求しているかを確認する。要求しているなら、両方向 round-trip が必要である。旧データを新システムで読めればよいだけなら、一方向の性質で十分なことがある。情報を落とす設計なら、落とす情報と残す情報を分け、残す情報だけを証明対象にする。特定条件でだけ戻せるなら、well-formed 条件を定義する。
...
\begin{softwarebox}
マイグレーションレビューでは、まず「互換性あり」という言葉を分解する。後方互換なのか、前方互換なのか、完全同型なのか、条件付きなのか、表示互換だけなのかを確認する。そのうえで、証明したい性質を一つか二つに絞る。
\end{softwarebox}
