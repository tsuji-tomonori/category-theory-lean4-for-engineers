...
図\ref{fig:ch39-bridge-map}では、「世界の中で論理を語る」という視点を中心に、圏論、Leanと型理論、仕様DSLとモデル検査の位置関係を示している。これらを同一の理論とみなす図ではなく、後続学習への接続を示す地図である。

\section{トポスを「集合のように振る舞う世界」として眺める}

本章で念頭に置くのは、有限極限、指数対象、部分対象分類子を持つelementary toposである。Grothendieck toposや内部言語の完全な意味論を同一の短い定義で置き換えない~\cite{maclane-moerdijk-sheaves}。
...
たとえば、マルチテナントのSaaSを考える。外側から見ると、システム全体には多数のテナント、権限、ユーザー、設定がある。しかし、あるテナントの内部で仕様を書くときには、「現在のテナントで見えるユーザー」「現在の権限で可能な操作」「現在の設定で有効な機能」だけを使って論理を組み立てることがある。これは文脈依存の仕様を理解するためのソフトウェア上の比喩であり、テナント分離そのものがトポスの内部言語だという主張ではない。
...
図\ref{fig:ch39-internal-language}は、外側から区別できる複数の文脈と、それぞれの内側で解釈される仕様の関係を表している。ここで重要なのは、「真偽は常に一つの固定された\texttt{Bool}だけで表される」と考えないことである。Leanでも \texttt{Bool} は計算できる二値データであり、\texttt{Prop} は証明の対象となる命題なので、両者は同じものではない。最終的な実装では\texttt{Bool}やエラー値として判定することが多いが、仕様設計では、どの文脈でどの命題を解釈しているかを分けておく必要がある。
...
このコードでは、\texttt{Spec}がDSLの構文であり、\texttt{holds}が意味論である。\texttt{uploadDSL}はDSL上の式であり、\texttt{holds uploadDSL u}は、その式が文脈\texttt{u}で成り立つというLeanの命題である。\texttt{uploadDSL\_correct} は、この解釈結果が前節の \texttt{CanUpload} と同値であることを確認し、DSLの構文と先に定めた仕様を接続している。

\begin{intuitionbox}
DSLの式は、まだ仕様そのものではない。式に意味論を与えて初めて、「この状態で成り立つ」「この状態では成り立たない」と言える。これは「構文に意味論を与える」という一般的な練習である。必要な圏論的意味論を構成していないため、トポスの内部言語のミニチュアまたは形式化とは呼ばない。
\end{intuitionbox}
...
図\ref{fig:ch39-spec-dsl-loop}では、DSL式を状態空間上の命題へ解釈し、その命題をLeanで証明するか、有限・抽象状態上でモデル検査するまでの流れを分けている。
...
\texttt{refines}は、本章の規約では「左の強い仕様が右の弱い仕様を含意する」という関係を表す。文献やツールによって精緻化順序の向きを逆に定めることもあるため、記号だけで向きを判断してはいけない。たとえば、「有効で、かつ残量がある」という仕様は、「有効である」という仕様より強い。このように、仕様間の関係を命題として扱えるようになると、仕様DSLは単なる設定ファイルではなく、検査可能な言語に近づく。
...
本章では、elementary toposを「集合の圏に似た構造を持ち、内部で論理を語れる圏」として眺めた。Leanの\texttt{Type}、\texttt{Prop}と本章の仕様DSLは、トポスの形式化ではない。DSLについて証明したのは、ここで定義した構文と\texttt{Semantics}の範囲だけである。トポスの内部言語へ進むには、有限極限、指数対象、部分対象分類子とcategorical semanticsを別途学ぶ必要がある。
