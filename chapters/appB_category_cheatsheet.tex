...
図\ref{fig:appB-category-cheatsheet-map}は、圏論の各用語を、対応しうる型・変換・設計上の問いへ結びつけている。これは用語の定義をソフトウェア比喩で置き換える図ではなく、本文のどこへ戻るかを探す索引である。
...
双対 & 矢印を逆に見た世界 & 入力側と出力側、作る側と使う側を反転して考える & 本書では矢印を反転して読む。Mathlibでは反対圏 \(C^{\mathrm{op}}\) \\
...
随伴 & Homの自然な対応 & 自由構成と忘却、抽象化と具体化 & 本書では \texttt{foldMap} の普遍性まで扱う \\
極限 & 図式に対する普遍的な錐 & 複数制約を満たす設定、整合するデータ統合 & 構造体と述語は具体例。一般定義には普遍性が必要 \\
...
\begin{softwarebox}
一般の圏の射は関数に限らない。マイグレーション、正規化、DTO 変換、encoder、decoder、adapter、validator なども、対象、合成、恒等射、法則を定めたモデルの中では射として見られる。ただし、副作用を含む任意の処理を、モデルを定めず自動的に射と呼べるわけではない。
\end{softwarebox}
...
\]
実務モデルでは、ここで射 \(f\) として表した処理について「adapter の前後で同じ結果になる」と読む。
\end{definitionbox}
...
\begin{softwarebox}
随伴は、余計な関係式を加えず構造を自由に生成する設計と、その構造を忘れて基礎データだけを見る設計を結びつける。たとえば、要素型からリストを作ることを free monoid の例として読むと、要素への関数がリスト全体の準同型へ一意に拡張される理由が見える。
\end{softwarebox}

\begin{warningbox}
この付録では、随伴の一般定義や単位・余単位の証明までは扱わない。第37章で直感を作り、必要になったら標準教材や Mathlib の \texttt{CategoryTheory} へ進む。
\end{warningbox}

\subsection{極限}

\begin{definitionbox}
極限とは、ある図式へ整合する射を持つ錐のうち、他の任意の錐から一意の射を受け取る普遍的な錐である。単に条件を満たす値が一つ存在するだけでは極限にならない。
\end{definitionbox}
...
\begin{definitionbox}
Pullback、または引き戻しは、二つの射が共通の対象へ向かうとき、その二射を可換にする普遍的な対象である。型と関数の圏では、その結果が一致するペアの型として表せる。
\end{definitionbox}
...
このコードは、付録Bで扱う用語をLeanの小さな定義へ対応させた一覧である。
\texttt{idFn}と\texttt{comp}は恒等射と合成、\texttt{Iso}は二方向の関数とround-trip性質、\texttt{optionToList\_natural}はadapterが\texttt{map}と干渉しないことを表している。
...
関手は恒等射と合成を保存する変換であり、自然変換は対象ごとの成分が自然性を満たす adapter である。
