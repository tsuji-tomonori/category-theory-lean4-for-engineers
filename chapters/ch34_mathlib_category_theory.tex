...
#check (Hom X Y)
#check (id X)
#check fun (f : Hom X Y) (g : Hom Y Z) => f >> g
...
theorem id_then (f : Hom X Y) : (id X) >> f = f := by
  simpa

theorem then_id (f : Hom X Y) : f >> (id Y) = f := by
  simpa
...
#check F.obj
#check F.map
#check fun {X Y : C} (f : Hom X Y) => F.map f
...
#check fun {X Y : C} (f : Hom X Y) => α.naturality f
...
theorem naturality_example {X Y : C} (f : Hom X Y) :
...
theorem iso_roundtrip_left : e.hom >> e.inv = id X := by
  simpa using e.hom_inv_id

theorem iso_roundtrip_right : e.inv >> e.hom = id Y := by
...
\texttt{MiniCategory.Hom X Y} & \(X \longrightarrow Y\) & 射の向きが一致しているか。 \\
...
\begin{summarybox}
\begin{itemize}
\item Mathlib の \texttt{Category C} は、対象型 \texttt{C} に圏構造を載せる型クラスとして読む。
\item \(X \longrightarrow Y\) は射、\(\mathbb{1}_X\) は恒等射、\texttt{f >> g} は矢印向きの合成である。
\item \(F : C \Rightarrow D\) は関手であり、\texttt{F.obj} と \texttt{F.map} によって対象と射を移す。
\item \texttt{NatTrans F G} は自然変換であり、\texttt{α.app X} と \texttt{α.naturality f} が中心になる。
\item \(X \cong Y\) は同型であり、\texttt{hom}、\texttt{inv}、二つの round-trip law を持つ。
\item Mathlib は強力だが、型クラス、universe、記法の負荷があるため、本書のミニ定義で概念を掴んでから進むのが安全である。
\item 実務では、Mathlib を使うこと自体が目的ではない。既存の標準定義と定理を再利用したいときに、必要な範囲で移行する。
\end{itemize}
\end{summarybox}
