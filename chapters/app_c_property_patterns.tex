...
このコードでは、二つの仕様\texttt{p}と\texttt{q}の強弱関係を\texttt{SpecLe p q}として表している。
反射律と推移律を定理として持つことで、仕様を弱める・段階的に変換するという判断をLean上で追える。
...
このコードでは、1件の移行関数\texttt{migrateItem}を\texttt{List.map}で全要素へ適用している。
\texttt{migrateItems\_preserves\_length}は、要素を変換してもリストの件数は変わらないことを帰納法で示している。
...
このコードでは、1件の値について成り立つround-trip性質を、リスト全体の\texttt{map}に持ち上げている。
証明はリストの形に沿った帰納法で進み、各要素では単体の変換規則を使っている。
...
このコードでは、口座残高が非負であるという不変条件を\texttt{NonNegative}として切り出している。
\texttt{deposit\_preserves\_nonnegative}は、入金操作の後もその不変条件が保たれることを示している。
...
このコードでは、失敗しうる処理の成功結果だけに事後条件を課している。
\texttt{withdrawChecked\_success\_post}は、\texttt{some newBalance}が返った場合、その値が期待する条件を満たすことを示す例である。
