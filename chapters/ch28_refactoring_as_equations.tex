...
\label{chap:ch28-refactoring-as-equations}
...
\FigurePlaceholder{fig-ch28-refactoring-equation-map.png}{0.90}{リファクタリングを等式として読む地図}{fig:ch28-refactoring-equation-map}
...
\end{minted}
\caption{関数抽出による意味保存}
\label{lst:ch28-extract-base-price}
\end{listing}
...
\FigurePlaceholder{fig-ch28-order-change.png}{0.88}{処理順序の入れ替えと可換性}{fig:ch28-order-change}
...
\label{lst:ch28-order-preserves}
\end{listing}
...
\label{lst:ch28-order-not-always-safe}
\end{listing}
...
\label{lst:ch28-commonize-preserves}
\end{listing}
...
\FigurePlaceholder{fig-ch28-option-bind-assoc.png}{0.88}{\texttt{Option.bind} の括弧付け変更}{fig:ch28-option-bind-assoc}
...
\label{lst:ch28-option-assoc-proof}
\end{listing}
...
\label{lst:ch28-option-pure-laws}
\end{listing}
...
\begin{summarybox}
\begin{itemize}
\item リファクタリングの安全性は、変更前後の意味保存として等式で表せる。
\item 関数抽出は、補助定義を展開すると元の式へ戻ることとして確認できる。
\item 処理順序の変更には、交換できる理由が必要である。足し算のように法則がある場合は、\texttt{calc} と \texttt{rw} で根拠を示せる。
\item 共通化では、旧関数が共通関数のどの分岐に対応するかを明示する。
\item wrapper削除は、wrapperが本当に恒等的である範囲でだけ安全である。
\item \texttt{Option.bind} の括弧付け変更や \texttt{some} の整理は、effectful code のリファクタリング規則として読める。
\item Leanで証明するのは、本番コード全体ではなく、壊れると困る小さな意味保存性でよい。
\end{itemize}
\end{summarybox}
