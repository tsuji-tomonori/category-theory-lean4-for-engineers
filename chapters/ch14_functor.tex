...
この章では、すぐに Mathlib の \texttt{CategoryTheory.Functor} へ進みません。
まず、型と関数の世界で \texttt{map} を観察し、関手則を Lean で確認します。
本格的な一般圏論へ進む前に、\texttt{map} が安全な設計語彙になる理由を捉えることが目標です。
...
同型性そのものは第12章で扱ったため、ここでは一件の変換をリスト全体へ持ち上げることに集中します。
...
この観点は、後の第30章で重要になります。
第30章では、一件のマイグレーションからリスト全体のマイグレーションへ進み、件数保存や合成保存を、より実務的なバッチ移行として扱います。
本章は、そのための基礎です。
...
本章では、関手を型と関数の世界で説明しました。
一般の圏から一般の圏への関手、反変関手、双関手、Mathlib の \texttt{CategoryTheory.Functor} などは扱っていません。
ここでは、\texttt{map} を「構造を保った一括変換」として読み、二つの法則を Lean で確認できることを目指しました。
...
次章では、関手どうしの間の変換である自然変換へ進みます。
そこで中心になるのは、adapter と業務ロジックの順序を入れ替えても結果が変わらないという自然性です。
本章の \texttt{map} と関手則は、その自然性を読むための前提になります。
