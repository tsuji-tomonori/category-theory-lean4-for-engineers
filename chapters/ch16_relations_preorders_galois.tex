...
\begin{summarybox}
\begin{itemize}
\item 関係は、二つの値が何らかの意味で対応していることを表す命題である。
\item 前順序は、反射性と推移性を持つ比較であり、情報量、権限、仕様の強弱、抽象度を表せる。
\item 単調性は、変換が比較関係を壊さないことを表す。
\item Galois 接続は、抽象化と具体化が順序を通じて対応していることを表す。
\item 情報を落とす変換は、同型ではなく、安全な近似として仕様化すべき場合がある。
\end{itemize}
\end{summarybox}
